...
#ma{text-align:left;width:90%;margin:0px auto 10px auto;background:#FFF;}
#ma img{width:100%;margin:0px auto 0px auto;}
#mab{height:55px;}
#ma1{width:150px;float:left;margin-right:12px;background:#FFF;margin-bottom:0px;}
#ma2{width:50%;float:left;text-align:left;line-height:150%;}
#ma2 h1{font-size:1.2em;font-weight:bold;}
#ma1 img,#ma2 img{height:160px;}
#ma1 span{font-size:0.5em;}
.clear{clear: both;}
.pad5{padding:5px;line-height:160%;}
#matitle{height:30px;background:#A4C23E;color:#FFF;text-align:center;line-height:30px;font-weight:bold;}
</style>
<!--菜单-->
...
sql="select * from Prod where prod_id="&id&" order by orderid,Prod_id desc"
...
<div id="ma"><div id="ma1"><img src="../<%=rs("prod_pic")%>" /></div><div id="ma2"><h1><%=rs("prod_name")%></h1><span><%if Rs("Prod_xb")=2 then%>女士<%else%>先生<%end if%><br />[ <%=rs("kid_name")%> ]</span></div><div class="clear"></div></div>
<div id="ma"><div id="matitle">感悟</div><div class="pad5"><%=Rs("Prod_gw")%></div></div><div id="ma"><div id="matitle">经历</div><div class="pad5"><%=Rs("Prod_jl")%></div></div><div id="ma"><div id="matitle">荣誉</div><div class="pad5"><%=Rs("Prod_ry")%></div></div><div id="ma" ><div id="matitle">作品</div><div class="pad5"></div></div>
