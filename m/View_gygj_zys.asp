...
#ma{text-align:center;width:90%;margin:0px auto 10px auto;background:#FFF;}
...
<div id="ma"><div id="ma1"><img src="../<%=rs("prod_pic")%>" /></div><div id="ma2"><h1><%=rs("prod_name")%></h1><span><%if oRs("Prod_xb")=2 then%>女士<%else%>先生<%end if%><br />[ <%=rs("kid_name")%> ]</span><br />感悟：<%=oRs("Prod_gw")%><br />经历：<%=oRs("Prod_jl")%><br />荣誉：<%=oRs("Prod_ry")%><br />作品：</div><div class="clear"></div></div>
