...
news.close
set news=nothing
response.write "<script language='javascript'>alert('提交成功');javascript:ToUrl('news.asp?yt="&newskinds&"');</script>"
else
...
-->
      ...
      <td width="500" height="25" class="hback" >
...
<textarea name="content" style="width:100%;height:450px;visibility:hidden;"><%=htmlspecialchars(htmlData)%></textarea> 
    ...
    <tr>
      <td height="22" align="right" class="back"> 造园师</td>
      <td class="hback_1"> 
                             
                             
                              <%set rsprod=server.createobject("adodb.recordset")
sqlprod="select * from prod order by Prod_Id desc"

rsprod.open sqlprod,conn,3,3
do while not rsprod.eof %>
                               

                 <div style="min-width:80px;float:left;">  <label> <input name="prod_id" type="radio" value="<%=rsprod("Prod_id")%>"<%if rsprod("Prod_id")=rs("Prod_id") then%> checked <%end if%>>
                   
                                    <%=rsprod("Prod_name")%></label></div>
