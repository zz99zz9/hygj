...
<!--#include file="leftmenu.asp"-->
			...
			<div class="row-fluid sortable">
				<div class="box span12">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-edit"></i> 产品修改</h2>
						<div class="box-icon">
 ...
 <form name="form1" method="post" action="Pro_save.asp?action=Upda" onSubmit="return checkform();" class="form-horizontal">
							<div class="control-group">
							  <label class="control-label" for="typeahead">标题 </label>
							  <div class="controls">
								<input class="input-xlarge focused" id="title" type="text" name="title" value="<%=Ors("Title")%>"><input name="yt" type="hidden" id="yt" value="<%=kinds%>">
							  </div>
                            </div>
                           <!--<div class="control-group"><label class="control-label" for="typeahead">产品编号</label>
                           <div class="controls">
                           <input name="Product_Id" type="text" id="Product_Id2" value="<%=oRs("Product_Id")%>" size="15" maxlength="15">
...
%>
          ...
          <%if ors("SmallClassName")="" then%>
           <option value="" selected>不指定小类</option>
          <%else%>
           <option value="<%=ors("SmallClassName")%>" selected><%=ors("SmallClassName")%></option>
            <%end if%>
            <%
...
%>
                           ...
                           <div class="control-group">
							  ...
							  <div class="controls">
								<input class="input-xlarge focused" id="DefaultPicUrl" type="text" name="DefaultPicUrl" value="<%=oRs("DefaultPicUrl")%>"> <a class="btn btn-success" href="#" onClick="window.open('Yr_UpFile.Asp?formname=form1&editname=DefaultPicUrl&uppath=images/guanggao&filelx=jpg','','status=no,scrollbaRs=no,top=20,left=110,width=420,height=165')">
...
<textarea name="content" style="width:100%;height:450px;visibility:hidden;"><%=htmlspecialchars(htmlData)%></textarea> 
                           ...
                           <div class="control-group">
							  ...
							  <div class="controls">
								<input class="input-xlarge focused" id="Orderid" type="text" value="<%=oRs("Orderid")%>" name="Orderid" onKeyup="if(this.value.length==17){document.all.cardid1.focus()}">
							  </div>
                            </div>
                             <div class="control-group">
							  
                              <div class="controls">
                              <input name="Passed" type="checkbox" id="Passed2" value="yes" <%if oRs("Passed")=True then%>checked<%End if%>>已通过审核 
                              </div>
                            </div>
                             <div class="control-group">
                             
                              <div class="controls">     
                              <input name="Elite" type="checkbox" id="Elite" value="yes" <%if oRs("Elite")=True then%>checked<%End if%>>推荐显示
      </div>
                            </div>
                            <div class="control-group">
							  <label class="control-label" for="typeahead">发布时间 </label>
							  <div class="controls">
								<input class="input-xlarge focused" id="UpdateTime" type="text" name="UpdateTime" value="<%=oRs("UpdateTime")%>" onKeyup="if(this.value.length==17){document.all.cardid1.focus()}">
							  </div>
                            </div>
